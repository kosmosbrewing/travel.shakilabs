    ...
    <div class="retro-panel-content space-y-3">
      ...
      <div v-for="row in rows" :key="row.name" class="space-y-1.5">
        <div class="flex items-baseline justify-between gap-3 text-caption">
          <span class="font-semibold" :class="row.totalCost === cheapest ? 'text-status-success' : 'text-foreground'">{{ row.name }}</span>
          <strong class="tabular-nums" :class="row.totalCost === cheapest ? 'text-status-success' : 'text-foreground'">{{ formatWon(row.totalCost) }}</strong>
        </div>
        <div class="h-3 overflow-hidden rounded-full bg-muted/55">
          ...
          <svg viewBox="0 0 100 12" preserveAspectRatio="none" class="block h-full w-full" aria-hidden="true">
            <rect :width="positiveBarWidth(row.totalCost, maximum)" height="12" :class="row.totalCost === cheapest ? 'fill-status-success' : 'fill-muted-foreground/45'" />
          </svg>
        </div>
